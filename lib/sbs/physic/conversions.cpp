...
constexpr double coef = -getConstant(Constant::R_PerfectGaz) / getConstant(Constant::G_MeanEarthGravityAcceleration) / getConstant(Constant::M_AirMeanMolarMass);

double getAltitude(double qnh, double measuredPressure, double measuredTemperature){
    // h = -R*T/(µ*g). ln(P/P0)
    return coef * (celsiusToKelvin(measuredTemperature)) * std::log(measuredPressure / qnh);
}

double computeQnh(double sensorAltitude, double measuredPressure, double measuredTemperature){
    // qnh = P * exp(µ*g*AltitudeRef / (R * T))
    return measuredPressure * std::exp(-sensorAltitude / coef / celsiusToKelvin(measuredTemperature));
}
